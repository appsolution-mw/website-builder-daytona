...
describe("mapSdkMessage", () => {
  it("system.init captures session_id without emitting", () => {
    const out = mapSdkMessage(
      { type: "system", subtype: "init", session_id: "s1", model: "claude-sonnet-4-6" } as any,
      ctx,
    );
  ...
  it("non-text content_block_delta is dropped (V1)", () => {
    const out = mapSdkMessage(
      { type: "stream_event", event: { type: "content_block_delta", delta: { type: "input_json_delta", partial_json: "{" } } } as any,
      ctx,
    );
    expect(out.events).toEqual([]);
  });
  ...
  it("assistant tool_use block → agent.tool_use", () => {
    const out = mapSdkMessage(
      { type: "assistant",
        message: { content: [{ type: "tool_use", name: "Read", input: { path: "x" } }] } } as any,
      ctx,
    );
  ...
  it("assistant text blocks are NOT mapped (only tool_use)", () => {
    const out = mapSdkMessage(
      { type: "assistant", message: { content: [{ type: "text", text: "blah" }] } } as any,
      ctx,
    );
    expect(out.events).toEqual([]);
  });
  ...
  it("user message (tool_result) is dropped — V1 doesn't forward results", () => {
    const out = mapSdkMessage(
      { type: "user",
        message: { content: [{ type: "tool_result", tool_use_id: "id", content: "ok" }] } } as any,
      ctx,
    );
    expect(out.events).toEqual([]);
  });
  ...
  it("missing usage on result still produces an event with zero tokens", () => {
    const out = mapSdkMessage(
      { type: "result", subtype: "success", duration_ms: 0, total_cost_usd: 0 } as any,
      ctx,
    );
  ...
  it("unknown message types produce no events", () => {
    const out = mapSdkMessage({ type: "something_unknown" } as any, ctx);
    expect(out.events).toEqual([]);
  });
