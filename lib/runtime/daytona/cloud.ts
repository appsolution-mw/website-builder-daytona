...
function buildBootCommand(args: {
  ...
  const setupSteps = [
    ...
    `wget -q --header=${shellQuote(`Authorization: token ${args.cloneToken}`)} -O repo.tar.gz ${shellQuote(`https://api.github.com/repos/${args.repoOwner}/${args.repoName}/tarball/${args.branch}`)}`,
  ...
  return `${setupSteps} && PROJECT_ID=${shellQuote(args.projectId)} BROKER_PORT=${BROKER_PORT} PREVIEW_PORT=${PREVIEW_PORT} nohup sh container/sandbox/entrypoint.sh > /workspace/entrypoint.log 2>&1 & sleep 3`;
}
