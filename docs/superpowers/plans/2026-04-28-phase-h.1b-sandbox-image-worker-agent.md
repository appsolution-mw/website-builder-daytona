...
- [x] **Step 3: Tag the phase**

Completed after the H.1b merge. The tag points at `bc158c9`, the last H.1b
implementation commit before the handover docs.

```bash
git tag -a phase-h.1b-sandbox-image-worker-agent bc158c9 -m "Phase H.1b: pre-built sandbox image + worker-agent + WorkerPoolRuntime"
```

- [x] **Step 4: Push main + tag**

`main` and `phase-h.1b-sandbox-image-worker-agent` were pushed to `origin`.
...
Verified: `main` is clean and synced with `origin/main`; the H.1b tag exists
