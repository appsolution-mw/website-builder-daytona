...
**Branch:** `feat/phase-h.1b-sandbox-image-worker-agent` — created in Task 1, merged + tagged in Task 15.
...
- [ ] **Step 2: Locate all importers / referrers of the old paths**
...
- [ ] **Step 3: Move the directories with `git mv`**
...
- [ ] **Step 5: Re-install workspace deps**
...
- [ ] **Step 7: Verify broker tests still pass**
...
- [ ] **Step 8: Verify dev server still boots cleanly**
...
- [ ] **Step 2: Create `worker-agent/tsconfig.json`**
...
- [ ] **Step 3: Create `worker-agent/vitest.config.ts`**
...
- [ ] **Step 4: Create a placeholder entry**
...
- [ ] **Step 5: Re-add `worker-agent` to `pnpm-workspace.yaml` if removed in Task 1**
...
- [ ] **Step 3: Implement `worker-agent/src/hmac.ts`**
...
- [ ] **Step 3: Implement `worker-agent/src/ports.ts`**
...
- [ ] **Step 3: Implement `worker-agent/src/docker.ts`**
...
- [ ] **Step 1: Define wire shapes in `worker-agent/src/types.ts`**
...
- [ ] **Step 4: Implement `worker-agent/src/server.ts`**
...
- [ ] **Step 3: Implement `worker-agent/src/heartbeat.ts`**
...
- [ ] **Step 1: Write `worker-agent/src/index.ts`**
...
- [ ] **Step 2: Create `worker-agent/Dockerfile`**
...
- [ ] **Step 3: Create `scripts/register-local-worker.ts`**
...
- [ ] **Step 4: Create `scripts/start-local-worker-agent.sh`**
...
- [ ] **Step 5: Create `scripts/build-sandbox-image.sh`** (the Dockerfile it references is created in Task 10)
...
- [ ] **Step 7: Add `dev:worker` to root `package.json`**
...
- [ ] **Step 9: Build and smoke-test the worker-agent image manually**
...
- [ ] **Step 1: Replace `container/sandbox/entrypoint.sh`**
...
- [ ] **Step 2: Create `container/sandbox/Dockerfile`**
...
- [ ] **Step 1: Define wire shapes in `lib/runtime/worker-pool/types.ts`**
...
- [ ] **Step 4: Implement `lib/runtime/worker-pool/agent-client.ts`**
...
- [ ] **Step 4: Implement `lib/runtime/worker-pool/runtime.ts`**
...
- [ ] **Step 5: Implement `lib/runtime/worker-pool/index.ts`**
...
- [ ] **Step 1: Update `lib/runtime/index.ts`**
...
- [ ] **Step 2: Update `lib/runtime/__tests__/index.test.ts`**
...
- [ ] **Step 1: Create `.github/workflows/sandbox-image.yml`**
...
- [ ] **Step 2: Create `.github/workflows/worker-agent.yml`**
...
- [ ] **Step 3: Push branch and verify CI fires**
...
- [ ] **Step 1: Dispatch the subagent with the prompt above (Task tool, type=general-purpose, model=sonnet)**
...
- [ ] **Step 3: Run the full host suite once more**
...
- [ ] **Step 4: Commit any inline fixes that came out of E2E**
